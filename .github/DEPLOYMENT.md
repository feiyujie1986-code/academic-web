...
使用 Docker 运行 Runner，通过卷挂载部署到宿主机目录。

### 1. 在 GitHub 获取 Runner Token

1. 进入仓库 `Settings` → `Actions` → `Runners`
2. 点击 `New self-hosted runner`
3. 复制页面中的 `TOKEN`（注意：Token 有效期较短，需及时使用）

### 2. 创建 Dockerfile

在服务器上创建 Runner 目录和 Dockerfile：
...
# 配置 runner（如果尚未配置）
if [ ! -f .runner ]; then
    ./config.sh --url "${GITHUB_URL}" --token "${GITHUB_TOKEN}" --name "${RUNNER_NAME:-docker-runner}" --labels "${RUNNER_LABELS:-self-hosted}" --unattended --replace
fi
...
docker run -d \
  --name github-runner \
  --restart always \
  -e GITHUB_URL="https://github.com/feiyujie1986-code/academic-web" \
  -e GITHUB_TOKEN="YOUR_TOKEN" \
  -e RUNNER_NAME="docker-runner" \
  -e RUNNER_LABELS="self-hosted" \
  -v /www/wwwroot/lms7799:/www/wwwroot/lms7799 \
...
| 参数                                           | 说明                                    |
| ---------------------------------------------- | --------------------------------------- |
| `-v /www/wwwroot/lms7799:/www/wwwroot/lms7799` | 挂载部署目录（宿主机:容器内路径需一致） |
| `GITHUB_TOKEN`                                 | 从 GitHub 获取的 Runner Token           |
| `RUNNER_NAME`                                  | Runner 名称（在 GitHub 显示）           |
...
# 在 GitHub 查看
# Settings → Actions → Runners 应显示 runner 为 "Idle" 状态
```
