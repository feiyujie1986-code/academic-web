...
import type { studentDataModel } from "@/api/member/student"
...
const tableData = ref<studentDataModel[]>([])
let activeRow: studentDataModel
...
// 重置密码
function resetPasswordAction(row: studentDataModel) {
    ...
    if (valid) {
      ...
      try {
        if (kind.value === "Add") {
          ...
          const res = await editStudentApi({
            id: activeRow.id,
            nickname: formData.nickname,
            remark: formData.remark,
            email: formData.email,
...
function deleteStudentAction(row: studentDataModel) {
  ElMessageBox.confirm("此操作将永久删除该用户, 是否继续?", "提示", {
    ...
    .then(() => {
      ...
      deleteStudentApi({ id: row.id }).then((res) => {
        if (res.code === 0) {
          ElMessage({ type: "success", message: res.msg })
...
function editDialog(row: studentDataModel) {
  activeRow = row
...
// ----- 列表行组长 switch -----
function isUserLeader(row: studentDataModel): boolean {
...
async function handleListLeaderChange(row: studentDataModel, val: boolean) {
  if (!row.organizationId) {
    ElMessage.warning("该会友尚未分配小组，无法设为组长")
    return
  }
            ...
            <template #default="scope">
              <div style="white-space: nowrap">
                ...
                <el-button
                  ...
                  icon="Delete"
                  size="small"
                  @click="deleteStudentAction(scope.row)"
