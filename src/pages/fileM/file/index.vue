...
import type { fileDataModel } from "@/api/fileM/file"
import { usePagination } from "@@/composables/usePagination_n"
import { formatDateTime } from "@@/utils/datetime"
import { reactive, ref } from "vue"
import { deleteApi, downloadApi, getFileListApi } from "@/api/fileM/file"
...
const path = ref(import.meta.env.VITE_BASE_URL)

function uploadSuccess(res: ApiResponseData<fileDataModel>) {
  if (res.code === 0) {
...
// 删除文件
function handleDelete(row: fileDataModel) {
  ElMessageBox.confirm("此操作将永久删除该文件, 是否继续?", "提示", {
    ...
    .then(async () => {
      const res = await deleteApi({ fileName: row.filename })
      if (res.code === 0) {
