...
onMounted(async () => {
  ...
  if (id) {
    try {
      const courseData = window.sessionStorage.getItem("courseData")
      if (courseData) {
        const course: courseListItem = JSON.parse(courseData)
        courseId.value = course.courseId
        formData.name = course.name
        formData.description = course.description
        formData.categoryId = course.categoryId
        formData.status = course.status
      } else {
        throw new Error("No course data in sessionStorage")
      }
    } catch (error) {
      console.log("解析课程数据失败，将从接口获取", error)
